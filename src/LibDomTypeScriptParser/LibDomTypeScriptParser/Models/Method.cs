...
public class Method: ICloneable
{
    ...
    public Method(ISignatureDeclaration method)
    {
        Name = method.Name.GetText();
        Documentation = new Documentation(method);

        ReturnType = new TsType(method.Type);
        Parameters = method.Parameters.Select(p => new TsType(p.Type)).ToList();
        ParameterNames = method.Parameters.Select(p => ((Identifier)p.Name).IdentifierStr).ToList();
        IsLastParameterParams = method.Parameters.LastOrDefault()?.Children.OfType<DotDotDotToken>().Any() ?? false;

        if (method.TypeParameters?.Any() ?? false)
        {
            TypeParameters = method.TypeParameters.Select(typeParamDecl => typeParamDecl.Children[0].IdentifierStr).ToList();
            foreach (var t in method.TypeParameters)
            {
                ...
                switch (t.Children.Count)
                {
                    case 2:
                        ...
                        {
                            constraintType = new TsType((ITypeNode)t.Children[1]);
                        }
                        else if (method.GetText().Contains('='))
                        {
                            defaultType = new TsType((ITypeNode)t.Children[1]);
                        }
                    break;
                    case 3:
                ...
                TypeParameterConstraints.Add(constraintType);
                TypeParameterDefaults.Add(defaultType);
            }
        }
    }

    public Method(CallSignatureDeclaration callSignature)
    {
        Name = "Invoke";
        Documentation = new Documentation(callSignature);

        ReturnType = new TsType(callSignature.Type);
        Parameters = callSignature.Parameters.Select(p => new TsType(p.Type)).ToList();
        ParameterNames = callSignature.Parameters.Select(p => ((Identifier)p.Name).IdentifierStr).ToList();

        if (callSignature.TypeParameters?.Any() ?? false)
        {
            TypeParameters = callSignature.TypeParameters.Select(typeParamDecl => typeParamDecl.Children[0].IdentifierStr).ToList();
            TypeParameterDefaults = callSignature.TypeParameters
                .Select(typeParamDecl => (typeParamDecl.Children.Count <= 1 || !typeParamDecl.Children.OfKind(SyntaxKind.EqualsToken).Any()) ? null : new TsType((ITypeNode)typeParamDecl.Children[1]))
                .ToList();
        }
    }
    ...
    public string ToString(Context context, bool generateAsClass = false, bool overrideExisting = false)
    {
        ...
        if (TypeParameterConstraints.Where(tc => tc != null).Any())
        {
            sb.Append(' ');
        ...
        else
        {
            ...
            {
                paramJsString = string.Join(", ", ParameterNames.SkipLast(1).Select(p => $"{{(global::Trungnt2910.Browser.JsObject.ToJsObjectString({p.ToCSharpParamName()}))}}"));
                if (!string.IsNullOrEmpty(paramJsString))
                {
                    paramJsString += ", ";
                }
                paramJsString += $"{{(string.Join(\",\", global::System.Linq.Enumerable.Select({ParameterNames[^1]} ?? global::System.Array.Empty<{paramsTypeString}>(), __arg => global::Trungnt2910.Browser.JsObject.ToJsObjectString(__arg))))}}";
            }
