...
public class Indexer
{
    ...
    public Indexer(IndexSignatureDeclaration declaration)
    {
        Documentation = new Documentation(declaration);
        Type = new TsType(declaration.Type);
        IsReadOnly = declaration.OfKind(SyntaxKind.ReadonlyKeyword).Any();
        Parameters = declaration.Parameters.Select(p => new TsType(p.Type)).ToList();
        ParameterNames = declaration.Parameters.Select(p => ((Identifier)p.Name).IdentifierStr).ToList();
    }
