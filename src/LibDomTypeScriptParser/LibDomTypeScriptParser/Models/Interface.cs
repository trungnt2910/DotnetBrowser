...
public class Interface
{
    ...
    public Interface(TypeLiteralNode declaration)
    {
        AddMembers(declaration, declaration.Members);
        Documentation = new Documentation(declaration);
        Documentation.AppendCommentLine($"Generated from inline type: <c>{declaration.GetText()}</c>.");
        var nameElements = string.Join("And", Methods.Select(m => m.Name.ToCSharpElementName())
            .Concat(Properties.Select(p => p.Name.ToCSharpElementName())));
        Name = $"InlineHas{nameElements}";
    }

    public Interface(InterfaceDeclaration declaration)
    {
        ...
        // with the same name.
        if (new[] {"Body", "InnerHTML"}.Contains(Name))
        {
            Name = "Has" + Name;
        }
        ...
        {
            TypeParameters = declaration.TypeParameters.Select(typeParamDecl => typeParamDecl.Children[0].IdentifierStr).ToList();
            TypeParameterDefaults = declaration.TypeParameters
                .Select(typeParamDecl => typeParamDecl.Children.Count <= 1 ? null : new TsType((ITypeNode)typeParamDecl.Children[1]))
                .ToList();
        }
        ...
        if (Properties.Where(p => p.Name == "length" && p.Type.Name == "number").Any())
        {
            ReadOnlyListElementType = Indexers.Where(i => i.Parameters.Count == 1 && i.Parameters[0].Name == "number").FirstOrDefault()?.Type;
        }
    }

    private void AddMembers(ITextRange declaration, IEnumerable<ITypeElement> members)
    {
        foreach (var member in members)
        {
            switch (member)
            {
                case PropertySignature propertyDeclaration:
                    Properties.Add(new Property(propertyDeclaration));
                    break;
                case MethodSignature methodDeclaration:
                    ...
                    {
                        if (methodDeclaration.IdentifierStr == "addEventListener")
                        {
                            EventMapType = new TsType(methodDeclaration
                                ...
                                .Children.OfType<TypeReferenceNode>().Single());
                        }
                    }
                    break;
                case IndexSignatureDeclaration indexDeclaration:
                    Indexers.Add(new Indexer(indexDeclaration));
                    break;
                case CallSignatureDeclaration callDeclaration:
                    CallSignatures.Add(new Method(callDeclaration));
                    break;
                case ConstructSignatureDeclaration constructSignatureDeclaration:
                    Console.Error.WriteLine($"[{declaration.Pos}-{declaration.End}] Construct signatures {constructSignatureDeclaration.GetText()} are not supported.");
                    break;
                default:
                    ...
                    else
                    {
                        foreach (var previousList in previousParams)
                        {
                            if (previousList.Count == currentList.Count && 
                                !previousList.Zip(currentList).Where(pair => !pair.First.EqualsIgnoreNullable(pair.Second)).Any())
                            {
                                collide = true;
                                break;
                            }
    ...
    private IEnumerable<Event> GetClassEvents(Context context, bool getInherited = false, bool ensureUnique = true, HashSet<Interface>? visitedInterfaces = null)
    {
        ...
        if (EventMapType != null)
        {
            ...
            if (eventMap == null)
            {
                Console.Error.WriteLine($"Cannot resolve event map: {EventMapType.Name}");
            }
            else
            {
    ...
    public string ToString(Context context, bool generateAsClass = false)
    {
        ...
        else
        {
            for (int nonDefaultTypeParams = TypeParameterDefaults.Count; nonDefaultTypeParams >= 0; --nonDefaultTypeParams)
            {
                ...
                if (!context.TypeExists(this))
                {
                    sb.AppendLine($"{context.Indent}[JsObject]");
                }
                else
                {
                    sb.AppendLine($"{context.Indent}// [JsObject]");
                ...
                else if (BaseTypes.Count == 0)
                {
                    if (CallSignatures.Count >= 1)
                    {
                        if (CallSignatures.Count > 1)
                        {
                            Console.Error.WriteLine("Multiple call signatures in one class are not supported. Generating the first signature only.");
                        }
                ...
                else
                {
                    if (CallSignatures.Count >= 1)
                    {
                        Console.Error.WriteLine("Call signatures are not supported in classes with a base type.");
                    }
                ...
                foreach (var p in GetClassProperties(context))
                {
                    ...
                    {
                        Console.Error.WriteLine($"[{Name.ToCSharpTypeName()}] Skipping {p.Type.Name} member {p.Name.ToCSharpElementName()}");
                        continue;
                    }

                    if (p.Name.ToCSharpElementName() == Name)
                    {
                        Console.Error.WriteLine($"[{Name.ToCSharpTypeName()}] Skipping member {p.Name.ToCSharpElementName()} because it has the same name as the enclosing class type.");
                        continue;
                    }
